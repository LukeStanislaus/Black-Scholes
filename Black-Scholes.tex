...
\end{equation}
Where in \egref\ref{riskfree}, $r$ represents the risk-free rate and $A$ the value of the 
...
or more precisely in a financial context, volatity. \egref\ref{randomprocess} is commonly 
written as 
\begin{equation}
...
$\mathrm{d}t$. Then we can further simplify \egref\ref{randomprocess} as 
\begin{equation}
...
\begin{proof}
    ...
    \end{equation}
    which in the limit of $\Delta x, \Delta t \to 0$, \egref\ref{taylorG} approaches

    \begin{equation}\label{limitG}
        \mathrm{d}G = \frac{\delta G}{\delta x}\mathrm{d}x + 
        \frac{\delta G}{\delta t}\mathrm{d}t
    \end{equation}
    Now we consider \egref\ref{limitG} to cover functions following Ito processes as 
    in \egref\ref{itoprocess}, i.e.

    \begin{equation}\label{itoprocessG}
        \mathrm{d}x = a(x,t)\mathrm{d}t + b(x,t)\mathrm{d}z
    \end{equation}

    and $G$ is a function of $x$ of time $t$. We can rewrite \egref\ref{itoprocessG} 
...
$$\Delta x^2 = b^2\Delta x.$$ 
 then we are done: using \egref\ref{limitG} 
\begin{align}
...
\begin{theorem}[Ito's Existence And Uniqueness Theorem] 
    ...
    \egref\ref{randomprocess} has ``strong solutions''. This means that for any standard 
    ...
    unique proccess $X_t = X_t^x$ which solves \egref\ref{randomprocess}. 
...
price $S$. Using \egref\ref{isosexistence} we can prove existence and uniqueness of 
\egref\ref{riskyasset}:

First we rewrite \egref\ref{riskyasset} in integral form:
\begin{equation}\label{riskyint}
    S(t) = S(0) + \mu\int_0^t \! S(u) \, \mathrm{d}u + \sigma\int_0^t \! S(u) \, 
    \mathrm{d}W(u).
\end{equation}

Then we rewrite our coefficients in the form of \egref\ref{randomprocess}: 
...
We then have that the unique solution to \egref\ref{riskyasset} is of the form 
\begin{equation}
    S(t) = S(0)exp\{\mu t - \frac{\sigma^2}{2}t + \sigma W(t)\}.
\end{equation}
This can be easily substituted into \egref\ref{riskyint} show that this solves \egref\ref{riskyasset}.
...
Where at \egref\ref{variance} we use the fact that 

\begin{equation*}
    \mathbb{E}(exp\{X\}) = exp\{\frac{1}{2}Var(X)\}
\end{equation*}

\egref\ref{result} then shows that if $\mu = 0$ we know that the expectation of S(t) is 
...
$\sigma$ to apply to our model. \egref\ref{mu} could suggest taking past values of the 
...
condition $t=T$ we appeal to the definition of a call as in \egref\ref{initialvalue}:
\begin{equation}
    C(T, z) = \max{(z-K, 0)}
\end{equation}

Now for our boundary conditions again we consider $u$ for $z=0$ and $z \to \infty$. Simiarly 
to \egref\ref{boundarycondition} we have 
\begin{align}
...
From \egref\ref{riskfree solution}, we get that 
\begin{align}
...
We begin by using \egref\ref{volatilityapprox} a calculate a value of $\sigma$. Using data from 
...
$z$. Considering a call option we have 
 $$ C(z,t) = z - (K + i)$$
 and then we calculate interest using \ref{riskfree solution} with negative $r$, so 
